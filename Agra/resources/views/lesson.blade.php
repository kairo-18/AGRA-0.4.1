...
<style>
    ...
    max-height: calc(1em * 3); /* Adjust based on font size and line count */
    white-space: normal;
    position: relative;
}
...
<!--=====================================End Navbar=====================================-->
<div id="transitionOverlay" class="fixed inset-0 flex items-center justify-center bg-blue-900 opacity-0 pointer-events-none transition-all duration-700 scale-150 rounded-full z-50"></div>
    ...
    <div class="innerDiv xl:flex bg-gray-50 h-full w-full rounded-lg xl overflow-auto">
        ...
        <div class="left-panel flex flex-col  bg-trnsparent h-screen w-full p-10">
            ...
            <div class ="lbl-course p-5 bg-transparent rounded-md">
                <h1 class="text-3xl font-bold text-blue-800">Welcome to {{$course->CourseName}}</h1>
            ...
            <div class="nav-section flex content-center bg-transparent h-16 w-full pl-2">
                <div class="text-xs font-medium text-center text-gray-500 border-b border-gray-200 dark:text-gray-400 dark:border-gray-700 w-full">
                    <ul class="flex flex-wrap -mb-px">
                        <li class="me-2">
                            <a href="/courses" class="inline-block p-4 border-b-2 border-transparent rounded-t-lg hover:text-gray-600 hover:border-blue-500 dark:hover:text-gray-300 text-md font-semibold">Courses</a>
                        </li>
                        <li class="me-2">
                            <a href="/courses/{{$course->id}}" class="inline-block p-4 text-blue-600 border-b-2 border-blue-600 rounded-t-lg active dark:text-blue-500 dark:border-blue-500 text-md font-semibold" aria-current="page">Lessons</a>
                        </li>
                        <li class="me-2">
                            <a class="inline-block p-4 border-b-2 border-transparent rounded-t-lg text-md font-semibold cursor-not-allowed ">Tasks</a>
                        </li>
                    </ul>
                </div>
            </div>
            <!--3 div Courses Content-->
            <div class = "learM-section flex flex-row flex-wrap justify-center items-start bg-gray-200  h-full xl:w-full w-full rounded-lg overflow-auto p-5 shadow-inner gap-5 gap-y-20">
                @foreach($lessons->sortBy('LessonName') as $lesson)
                    <a href="/lessons/{{$course->id}}/{{$lesson->id}}" class="yt-vid w-[18rem] h-[13rem] focus:outline-none transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 duration-300 bg-white shadow-md rounded-xl lesson-card">
                        <div class="h-1/5">
                            ...
                            <div class ="w-full p-3 bg-white rounded-xl">
                                ...
                                <h1 class="font-bold text-md text-blue-800">{{$lesson->LessonName}}</h1>
                                <p class="font-normal text-sm text-blue-800 line-clamp">{{$lesson->LessonDescription}}</p>
                            </div>
                        </div>
                    </a>
